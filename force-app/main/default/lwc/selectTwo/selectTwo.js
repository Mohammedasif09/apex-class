...
export default class SelectTwo extends LightningElement {

    // @api selectedValueQueue;
    // @api selectedValueSkillsFields;
    @api selectedValueSkills;
    ...
    set options(items) {
        ...
        console.log('set value',this.allOptions);
        console.log('set allSelectedItem',this.allSelectedItem);
        setTimeout(()=>{
            //console.log('selectedValueQueue1',JSON.parse(JSON.stringify(this.selectedValueQueue)));
            console.log('selectedValueQueue2',JSON.parse(JSON.stringify(this.selectedValueSkills)));
            //console.log('selectedValueQueue3',JSON.parse(JSON.stringify(this.selectedValueSkillsFields)));
            //if(this.selectedValueQueue.length || this.selectedValueSkillsFields.length || this.selectedValueSkills.length){
            // if(this.selectedValueSkillsFields.length){
            //     //this.setPicklistselectedValueFields();
            //     console.log('selectedValueQueue.Field_Name__c',JSON.parse(JSON.stringify(this.selectedValueSkillsFields)));
            // }
            if(this.selectedValueSkills.length){
                this.setPicklistselectedValueSkills();
                //console.log('selectedValueQueue.Skill_Name__c',JSON.parse(JSON.stringify(this.selectedValueSkills)));
            }
            // if(this.selectedValueQueue.length){
            //     this.setPicklistselectedValueQueue();
            // }
        //}
        },2000)
    ...
    setPicklistselectedValueQueue(){
        this.allOptions.forEach(ele=>{
            this.selectedValueQueue.forEach(element=>{
                if(ele.label == element.Name){
                    ele.selected = true;
                }
            })
        })
        console.log('element-selectedValueQueue',this.allOptions);
        this.allSelectedItem = JSON.parse(JSON.stringify(this.allOptions.filter(result => result.selected)));
        console.log('element=>',this.allSelectedItem);
        this.setQueuePicklistLabels();

    }

    setPicklistselectedValueFields(){
        this.allOptions.forEach(ele=>{
            this.selectedValueQueue.forEach(element=>{
                if(ele.label == element.value){
                    ele.selected = true;
                }
            })
        })
    ...
    setPicklistselectedValueSkills(){
        this.allOptions.forEach(ele=>{
            this.selectedValueQueue.forEach(element=>{
    ...
    setQueuePicklistLabels(){
        console.log('statment Method Call');
        let selectedQueueArr = [];
        this.allOptions.forEach(ele=>{
            if(ele.selected){
                selectedQueueArr.push(ele.label);

            }
        })
        console.log('selectedQueueArr',selectedQueueArr);
        if(selectedQueueArr.length < this.maximumLength){
            this.selectedQueuesTwo = selectedQueueArr[0];
        }
        else{
            console.log('inside');
            if(selectedQueueArr.length > this.maximumLength){
                console.log('selectedQueueArr[0]+selectedQueueArr[1]',selectedQueueArr[0]+selectedQueueArr[1]);
                console.log('selectedQueueArr.length',selectedQueueArr.length);
                this.selectedQueuesTwo = selectedQueueArr[0]+', '+selectedQueueArr[1]+', ( '+String(selectedQueueArr.length - 2)+' More Items. )';
            }
            else{
                this.selectedQueuesTwo = selectedQueueArr[0]+', '+selectedQueueArr[1];
            }
        }
        console.log('selectedQueuesTwo',this.selectedQueuesTwo);
    }
    // @api selectedValue(value){
    //     console.log('value 2 value',this.allOptions);
