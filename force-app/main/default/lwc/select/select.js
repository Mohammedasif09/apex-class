...
export default class Select extends LightningElement {
    ...
    @api selectedObjFieldValue;
    @api serviceUserRecordId;
    ...
    set options(items) {
        ...
        if (this.selectedValueSkillsFields) {
            let parentValue = this.selectedValueSkillsFields;
            this.setPicklistselectedValueSkillsFields(parentValue);
        }
        if (this.selectedValueSkills) {
            let parentValue = this.selectedValueSkills;
            this.setPicklistselectedValueSkills(parentValue);
        }
        if (this.selectedValueQueue) {
            let parentValue = this.selectedValueQueue;
            this.setPicklistselectedValueQueues(parentValue);
        }
        if (this.selectedObjFieldValue) {
            let parentValue = this.selectedObjFieldValue;
            this.setPicklistselectedFieldsValue(parentValue);
        }
        if(this.serviceUserRecordId){
            let parentValue = this.serviceUserRecordId;
            this.setServiceUserValue(parentValue);
        }
    }

    setPicklistselectedValueQueues(items) {
    ...
    setPicklistselectedValueSkills(items) {
        this.allOptions.forEach(ele => {
            items.forEach(element => {
                if (ele.label == element.Skill_Name__c) {
                    ele.selected = true;
                }
            })
        })
    ...
    setPicklistselectedValueSkillsFields(items) {
        this.allOptions.forEach(ele => {
            items.forEach(element => {
                if (ele.value == element.Field_Name__c) {
                    ele.selected = true;
                }
            })
        })
    ...
    setPicklistselectedFieldsValue(items) {
        this.allOptions.forEach(ele => {
            items.forEach(element => {
    ...
    setServiceUserValue(items) {
        ...
        this.allSelectedItem = JSON.parse(JSON.stringify(this.allOptions.filter(result => result.selected)));        
        console.log('this.allSelectedItem',this.allSelectedItem);
        //this.setQueuePicklistLabels();
    }
